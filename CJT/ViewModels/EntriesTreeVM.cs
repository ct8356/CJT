...
namespace CJT.ViewModels {
    public abstract class EntriesTreeVM : ITreeVM {
        ...
        public EntriesTreeVM(IMainVM parentVM) {
            ...
            TypePanelVM.Objects = new ObservableCollection<object> {
                typeof(Entry), typeof(Note), typeof(PartClass), typeof(PartInstance), typeof(Task), typeof(Tag) };
        ...
        public void ChooseAndProcessEntries() {
            if (TypePanelVM.SelectedObject != null) {
                ...
                if (selectedType == typeof(Note))
                    processEntries<Note>(DbContext.Notes);
